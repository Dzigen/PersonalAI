...
def qd_custom_formate(query: str) -> str:
    """Функция готовит контекст для промпта задачи декомпозиции запроса.
...
def qd_custom_postprocess(parsed_response: List[str], **kwargs) -> List[str]:
    """Функция предназначена для постобработки разобранного ответа LLM-агента в задаче декомпозиции запроса.
