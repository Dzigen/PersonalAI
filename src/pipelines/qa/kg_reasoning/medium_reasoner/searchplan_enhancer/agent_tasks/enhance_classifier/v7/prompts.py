...
2. If [Next search-plan queries at now] have queries, which contains undefined/uncertain or relative key words (in squared brackets, for example), that can be clarified by the information from [Complited search-plan queries], then return 'True' as [Answer].
3. Every individual query of the search plan in [Next search-plan queries at now] must be information-consistent and contains all required knowledge to be executed independently from the other plan queries and without internal intent/request distortion.
...
8. If "Existing search-plan is complited. Next search-steps need to be planned." in [Next search-plan queries at now], then return 'True' as [Answer].
...
What award did [director's name] win?
[Answer #5]
True
'''
...
2. Если [Next search-plan queries at now] содержат запросы, включающие неопределенные ключевые слова или местоимения (в квадратных кавычка, например), которые можно уточнить с помощью информации из [Complited search-plan queries], то верните 'True' в качестве [Answer].
3. Каждый отдельный запрос в плане поиска из [Next search-plan queries at now] должен быть информационно-согласованным и содержать все необходимые знания для независимого выполнения от других запросов плана и без искажения внутреннего намерения.
...
8. Если "Existing search-plan is complited. Next search-steps need to be planned." в [Next search-plan queries at now], то верните 'True' в качестве [Answer].
...
[Next search-plan queries at now #5]
Какую награду получил [имя режиссера]?
[Answer #5]
True
'''
