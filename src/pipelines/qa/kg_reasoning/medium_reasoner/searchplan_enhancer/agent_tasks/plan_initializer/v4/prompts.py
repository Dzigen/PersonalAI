...
3. Every individual query of the search plan must be information-consistent and contains all required knowledge to be executed independently from the other plan queries and without internal intent/request distortion.
4. If the original question cannot be represented as independent search steps, then the plan consists of a single step - that question itself.
5. Do not use any external knowledge. Rely only on the [Base question].
6. Preserve exact numbers, units of measurement, and dates.
7. Return your response in english.
...
3. Каждый отдельный запрос в плане поиска должен быть информационно-согласованным и содержать все необходимые знания для независимого выполнения от других запросов плана и без искажения внутреннего намерения.
4. Если исходный вопрос нельзя представить в виде независимых шагов поиска, тогда план состоит из единственного шага - этого вопроса.
5. Не используйте внешние знания. Опирайтесь только на [Base question].
6. Сохраняйте точные числа, единицы измерения и даты.
7. Верни свой ответ на русском языке.
