...
6. If [Next search-plan queries at now] have queries, which contains undefined key words, that can be clarified by the information from [Complited search-plan queries], then enhance them.
7. Preserve exact numbers, units of measurement, and dates.
...
6. Если [Next search-plan queries at now] содержат запросы, включающие неопределенные ключевые слова, которые можно уточнить с помощью информации из [Complited search-plan queries], то улучшите/переформулируйте их.
7. Сохраняйте точные числа, единицы измерения и даты.
