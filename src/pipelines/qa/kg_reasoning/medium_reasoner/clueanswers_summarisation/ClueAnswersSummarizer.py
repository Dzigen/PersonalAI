...
class ClueAnswersSummarizerConfig(BaseComponentConfig, LanguageConfig):
    ...
    :param agent_tasks_config: Конфигурации LLM-промптов для решения заданных задач с помощью LLM-агента. Значение по умолчанию ClueAnswersSummarizerAgentTasksConfig().
    ...
    log_path: str = CQSUMM_MAIN_LOG_PATH
...
class ClueAnswersSummarizer(CacheUtils, AgentStatOperations, CacheOperations):
    ...
    def perform(self, search_query: str, clue_queries: List[str], clue_answers: List[str]) -> Tuple[str, ReturnInfo, CompositeModuleDetailedResult]:
        ...
        :param clue_queries: Список Clue-запросов на естественном языке.
        ...
        """
        self.log.debug("START CLUE-QUERIES SUMMARISATION...", verbose=self.verbose, log_level=self.log_level)
        ...
        self.log.debug("* Clue-queries: %s", clue_queries, verbose=self.verbose, log_level=self.log_level)
        self.log.debug("* Clue-answers: %s", clue_answers, verbose=self.verbose, log_level=self.log_level)
        ...
        self.log.debug("RESULT: %s", summ_answer, verbose=self.verbose, log_level=self.log_level)
