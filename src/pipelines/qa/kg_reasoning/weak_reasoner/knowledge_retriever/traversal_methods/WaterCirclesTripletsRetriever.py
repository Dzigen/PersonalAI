...
def process_chain(
    ...
    for triplet in chain:
        subj = triplet[0]
        subj_no_props = {key: value for key,
                         value in subj.items() if key != "prop"}
        ...
        obj = triplet[2]
        obj_no_props = {key: value for key,
                        value in obj.items() if key != "prop"}
        obj_props = obj["prop"]
...
class WaterCirclesRetriever(AbstractTripletsRetriever, CacheUtils):
            ...
            for triplet_raw in res:
                ...
                if triplet not in new_chain:
                    ...
                    prop_values = [val.lower()
                                   for val in triplet[1]["prop"].values()]
                    cnt1 = 0
                ...
                if "forw" in direction:
                    new_entities.append(
                        (triplet_raw.end_node.name, "", "node"))
    ...
    def get_relevant_triplets(self, query_info: QueryInfo, depth: int = 1) -> List[Triplet]:
        ...
        triplets_dict, inters_chains1, inters_chains2 = self.bfs(
            seed_entities, depth)
        ...
        for seed_entity in inters_chains1:
            ...
            ent_inters_chains1 = sorted(
                ent_inters_chains1, key=lambda x: x[1], reverse=True)

            inters_chains1_more = [ch for ch,
                                   cnt in ent_inters_chains1 if cnt > 1]
            inters_chains1_less = [ch for ch,
                                   cnt in ent_inters_chains1 if cnt == 1]
            ...
            if self.config.strict_filter:
                ...
                if not ent_chain_triplets:
                    ent_chain_triplets = chain_triplets1_more + \
                        chain_triplets1_less + chain_triplets2
            else:
                ent_chain_triplets = chain_triplets1_more + \
                    chain_triplets1_less + chain_triplets2
        ...
        def format_triplet(triplet_data):
            ...
            subj_node = NodeCreator.create(
                name=subj["name"], n_type=subj["type"], prop=subj["prop"])
            subj_node.id = subj['id']

            obj_node = NodeCreator.create(
                name=obj["name"], n_type=obj["type"], prop=obj["prop"])
            obj_node.id = obj['id']

            rel_edge = RelationCreator.create(name=rel.get(
                "name", None), r_type=rel["type"], prop=rel["prop"])
            rel_edge.id = rel["id"]
        ...
        def triplet_from_hyper(text, seed_entity, subj_props, obj_props, rel_props, rel_id):
            subj_node = NodeCreator.create(
                name=seed_entity, n_type=NodeType.object, prop=subj_props)
            subj_node.id = '1'

            obj_node = NodeCreator.create(
                name=text, n_type=NodeType.hyper, prop=obj_props)
            obj_node.id = '1'

            rel_edge = RelationCreator.create(
                r_type=RelationType.hyper, prop=rel_props)
            rel_edge.id = rel_id
        ...
        for text, seed_entity, subj_props, obj_props, rel_props, rel_id in output_texts_hyper:
            triplet = triplet_from_hyper(
                text, seed_entity, subj_props, obj_props, rel_props, rel_id)
