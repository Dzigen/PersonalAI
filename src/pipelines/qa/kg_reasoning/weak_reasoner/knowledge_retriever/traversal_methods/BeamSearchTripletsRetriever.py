    ...
    def graph_beamsearch(self, query: str, node: NodeInfo) -> List[TraversedPath]:
        ...
        for cur_depth in range(self.config.max_depth):
            ...
            for i in range(len(traversing_paths)):
                ...
                self.log.debug(f"Смежные вершины: %d", len(adj_nodes), verbose=self.verbose, log_level=self.log_level)

                if len(adj_nodes) < 1:
                    self.log.debug("У tail-вершины нет смежных вершин. Считаем путь завершившимся.", verbose=self.verbose, log_level=self.log_level)
