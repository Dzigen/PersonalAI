...
class KuzuTreeConnector(AbstractTreeDatabaseConnection):
    ...
    def check_consistency(self) -> None:
        ...
        leafs_wo_strid = self.conn.execute(
        ...
        summarized_wo_childs = self.conn.execute(
            "MATCH (parent:summarized) WHERE COUNT { (parent)-[rel:relation]->() } < 1 RETURN parent;")
        assert len(summarized_wo_childs) < 1
        # нет leaf-вершин c детьми
        leafs_with_childs = self.conn.execute(
            "MATCH (parent:leaf) WHERE COUNT { (parent)-[rel:relation]->() } > 1 RETURN parent;")
    ...
    def create_node_query(self, node: TreeNode) -> str:
        ...
        for prop_name, prop_value in node.props.items():
            if prop_name == 'str_id' and node.type == TreeNodeType.leaf:
                fields['str_id'] = json.dumps(
                    node.props['str_id'], ensure_ascii=False)
    ...
    def update(self, items: List[TreeNode]) -> None:
        ...
        for cur_old_item in old_items:
            ...
            grouped_pnodes = {k: [item[0] for item in g]
                              for k, g in groupby(pnodes_info, key=lambda x: x[1])}
            ...
            grouped_cnodes = {k: [item[0] for item in g]
                              for k, g in groupby(cnodes_info, key=lambda x: x[1])}
    ...
    def delete(self, ids: List[str], ids_type: TreeIdType = TreeIdType.external) -> None:
        ...
        for id in ids:
            self.conn.execute(
                f'MATCH (parent)-[rel:relation]->(leaf) WHERE leaf.{ids_type.value} = "{id}" DELETE rel;')
            self.conn.execute(
    ...
    def formate_nodes_output(self, output: object) -> List[TreeNode]:
        ...
        for i in range(nodes_count):
            raw_node = output['n'][i]
            node_type = TREENODES_TYPES_MAP[self.config.params['table_type_map']
                                            ['nodes']['inverse'][raw_node['_label']]]
    ...
    def get_leaf_descendants(self, id: str, id_type: TreeIdType = TreeIdType.external) -> List[TreeNode]:
        ...
        raw_output = self.conn.execute(
            f'MATCH (ancestor)-[:relation*0..]->(n:leaf) WHERE ancestor.{id_type.value} = "{id}" RETURN n;')
