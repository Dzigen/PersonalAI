...
class KuzuGraphConnector(AbstractGraphDatabaseConnection):
    ...
    def delete(self, ids: List[str], delete_info: Dict[int, Dict[str, bool]] = dict()) -> None:
        ...
        for i, t_id in enumerate(ids):
            ...
            if len(nodes_to_delete) > 0:
                ...
                self.execute(f'MATCH (n) WHERE {where_statement} DELETE n;')
    ...
    def get_incident_triples(self, base_node: NodeInfo,
        ...
        output = self.execute(
            f'MATCH (n1:{base_node.type.value})-[rel{str_accepted_relations}]-(n2{str_accepted_nodes}) \
                WHERE n1.str_id = "{base_node.id}" RETURN n1,rel,n2;')
