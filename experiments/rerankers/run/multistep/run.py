print("Start MultiStepReranker evaluation...")
import sys
import joblib
import yaml
...
from src.rerankers.methods.MultiStepReranker import MultiStepReranker, MultiStepRerankerConfig, RerankStep, MS_RERANKING_TYPES_MAP
...
RAW_MULTISTEP_CONFIG = EXP_PARAMS['multistep_config']
reranking_sequence = [
    RerankStep(type=MS_RERANKING_TYPES_MAP[raw_config['type']], name=raw_config['name'], 
               fetch_n=raw_config['fetch_n'], extended_params=raw_config['extended_params']) 
    for raw_config in RAW_MULTISTEP_CONFIG]

reranker_config = MultiStepRerankerConfig(
    reranking_sequence=reranking_sequence
)

print("MultiStepRerankerConfig:")
