...
def explore_neighbors(kg_model: KnowledgeGraphModel, query: str, current_node: NodeInfo,
    ...
    for neighbor in adjecent_nodes:
        ...
        raw_output = kg_model.graph_struct.db_conn.get_nodes_shared_ids(
            current_node, neighbor, id_type='r_id')
    ...
    if SPECEXP_PARAMS['RETRIEVE_SETTING']['max_triples_per_node'] != -1:
        ...
        retrieved_triple_rids = list(map(lambda item: item.id, filtered_triples))
    else:
        retrieved_triple_rids = incident_triple_rids
    ...
    for neighbor in topk_neighbor_nodes:
        retrieved_triple_rids.update(explore_neighbors(
...
for pack_name, queries in queries_packs:
    ...
    for i in range(len(queries)):
        ...
        print(f"RETRIEVED TRIPLES IN TOTAL (AFTER FILTERING): {len(retrieved_triples)} {triple_types_frequency}\n{'\n'.join(retrieved_triples)}")
