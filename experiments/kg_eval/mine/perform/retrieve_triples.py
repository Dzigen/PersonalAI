...
DATASET_PATH = f"{EXPDIR_PARAMS['WORKSPACE_CONTAINER_DIRS']['base_path']}/{EXPDIR_PARAMS['WORKSPACE_CONTAINER_DIRS']['qa_datasets']}/{SPECEXP_PARAMS['DATASET_NAME']}"
    ...
    for i in range(len(queries)):
        ...
        for object_node in matched_objects:
            adjecent_nodes: List[NodeInfo] = kg_model.graph_struct.db_conn.get_adjecent_nodes(
                object_node, FORMATED_ACCEPTED_N_TYPES)
            ...
            for adjecent_node in adjecent_nodes:
                raw_output = kg_model.graph_struct.db_conn.get_nodes_shared_ids(
                    object_node, adjecent_node, id_type='r_id')
                incident_triple_rids.update(list(map(lambda item: item['r_id'], raw_output)))

            if SPECEXP_PARAMS['RETRIEVE_SETTING']['max_triples_per_node'] != -1:
                filtered_triples = triples_retriever.run(
                    queries[i], top_k=SPECEXP_PARAMS['RETRIEVE_SETTING']['max_triples_per_node'],
                    subset_ids=list(incident_triple_rids))
                filtered_triple_rids = list(map(lambda item: item.id, filtered_triples))
            else:
                filtered_triple_rids = incident_triple_rids

            print(f"FILTERED INCIDENT TRIPLES FOR NODE {object_node}: {len(filtered_triple_rids)} {filtered_triple_rids}")

            retrieved_triple_rids.update(filtered_triple_rids)
        ...
        triple_types_frequency = dict(Counter([triple.relation.type for triple in retrieved_triples]))
        print(f"RETRIEVED TRIPLES IN TOTAL: {len(retrieved_triples)} {triple_types_frequency}\n{'\n'.join(retrieved_triples)}")
...
for pack_name, queries in queries_packs:
    ...
    for tmp_dump in tqdm(tmp_triples_dumps):
        ...
        retrieved_triples: List[Triplet] = triples_info['retrieved_triples']
        str_triples = [TripletCreator.stringify(triple) for triple in retrieved_triples]

        accum_triples[answer_num] = {
            'query': queries[answer_num],
            'retrieved_stringified_triples': str_triples
        }
