...
GENERAL_QAPIPE_CONFIGS = list(product(
    ...
    AVAIL_E2NMATCHER_CONFIGS, # 4
    
    AVAIL_CQUERIES, # 5
    
    AVAIL_KRETR_CONFIGS, # 6
    AVAIL_KFILTR_CONFIGS, # 7
))
...
for ds_idx, dataset in enumerate(AVAILABLE_DATASET_NAMES):
    ...
    for kg_idx, kg_name in enumerate(AVAILABLE_KG_NAMES):
        ...
        for spec_raw_params in GENERAL_QAPIPE_CONFIGS:
            ...
            SPECIFIC_QA_PARAMS['kg_reasoner']['reasoner_config']['e2n_matcher_config'] = spec_raw_params[4]

            SPECIFIC_QA_PARAMS['kg_reasoner']['reasoner_config']['cluequeries_generator_config']['max_cqueries_amount'] = spec_raw_params[5]

            SPECIFIC_QA_PARAMS['kg_reasoner']['reasoner_config']['knowledge_retriever_config']['retriever_method'] = spec_raw_params[6][0]
            SPECIFIC_QA_PARAMS['kg_reasoner']['reasoner_config']['knowledge_retriever_config']['retriever_config'] = spec_raw_params[6][1]
            
            SPECIFIC_QA_PARAMS['kg_reasoner']['reasoner_config']['knowledge_retriever_config']['filter_method'] = spec_raw_params[7][0]
            SPECIFIC_QA_PARAMS['kg_reasoner']['reasoner_config']['knowledge_retriever_config']['filter_config'] = spec_raw_params[7][1]
