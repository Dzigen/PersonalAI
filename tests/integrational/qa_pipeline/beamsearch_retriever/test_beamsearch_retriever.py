...
def get_available_nids(base_nid: str, cur_path_idx: int, traversing_paths: List[TraversingPath], prev_nid: str,
    ...
    try:
        real_adjn = bm_retriever.get_available_nids(
            base_nid, cur_path_idx, traversing_paths, prev_nid)
    except ValueError:
        assert exception
    else:
        assert not exception
...
def test_get_available_rinfo(
    ...
    try:
        real_shared_tinfo, real_ridstotids_map = bm_retriever.get_available_rinfo(
            base_nid, adj_nids, cur_path_idx, traversing_paths)
    except ValueError:
        assert exception
    else:
        assert not exception
        ...
        assert set(real_shared_tinfo.keys()) == set(
            expected_shared_tinfo.keys())
        ...
        assert set(real_ridstotids_map.keys()) == set(
            expected_ridstotids_map.keys())
