...
KEY_HASH1 = hashlib.sha1(''.join(list(map(lambda k: hashlib.sha1(
    k.encode()).hexdigest(), KEY1))).encode()).hexdigest()

KEY1_ORDER_MODIF = ["123", DEFAULT_OLLAMA_CONFIG.to_str(), "asd", "asd"]
KEY_HASH1_ORDER_MODIF = hashlib.sha1(''.join(list(map(lambda k: hashlib.sha1(
    k.encode()).hexdigest(), KEY1_ORDER_MODIF))).encode()).hexdigest()
...
KEY_HASH1_OBJECT_MODIF1 = hashlib.sha1(''.join(list(map(lambda k: hashlib.sha1(
    k.encode()).hexdigest(), KEY1_OBJECT_MODIF1))).encode()).hexdigest()
...
KEY_HASH1_OBJECT_MODIF2 = hashlib.sha1(''.join(list(map(lambda k: hashlib.sha1(
    k.encode()).hexdigest(), KEY1_OBJECT_MODIF2))).encode()).hexdigest()
...
CACHEKV_SAVE_TEST_CASES = [
    ...
    (CACHED_INSTANCES, VALUE3, KEY1_OBJECT_MODIF2,
     None, KEY_HASH1_OBJECT_MODIF2, False),
    # 4. непустое key_hash-значение (values-отсутствует)
    (CACHED_INSTANCES, VALUE3, None, KEY_HASH1_OBJECT_MODIF2,
     KEY_HASH1_OBJECT_MODIF2, False),
...
for db_vendor in AVAILABLE_KV_DBS:
    for i in range(len(CACHEKV_SAVE_TEST_CASES)):
        CACHEKV_POPULATED_SAVE_TEST_CASES.append(
            CACHEKV_SAVE_TEST_CASES[i] + (db_vendor,))
...
CACHEKV_LOAD_TEST_CASES = [
    ...
    (CACHED_INSTANCES, KEY1_OBJECT_MODIF2,
     None, -1, KEY_HASH1_OBJECT_MODIF2, False),
    # 6. непустое key_hash-значение (value-отсутвтует)
    (CACHED_INSTANCES, None, KEY_HASH1_OBJECT_MODIF2, -
     1, KEY_HASH1_OBJECT_MODIF2, False)
]

CACHEKV_POPULATED_LOAD_TEST_CASES = []
for db_vendor in AVAILABLE_KV_DBS:
    for i in range(len(CACHEKV_LOAD_TEST_CASES)):
        CACHEKV_POPULATED_LOAD_TEST_CASES.append(
            CACHEKV_LOAD_TEST_CASES[i] + (db_vendor,))
