...
def test_load_value(cache_instances: List[KeyValueDBInstance], key: List[object], key_hash: str,
    ...
    try:
        real_status, real_hash_key, real_value = cachekv_conn.load_value(
            key, key_hash)
        if real_status != 0:
...
def test_save_value(cache_instances: List[KeyValueDBInstance], value: object, key: List[object], key_hash: str,
    ...
    try:
        real_key_hash = cachekv_conn.save_value(
            value, key=key, key_hash=key_hash)
    except ValueError as e:
        assert exception
    else:
        assert not exception
        ...
        real_value = pickle.loads(
            cachekv_conn.kv_conn.read([real_key_hash])[0].value)
