...
def redis_cache_conn():
    redis_config = KVDBConnectionConfig(host='localhost', port=6381, need_to_clear=True, db_info={'db': 0, 'table': 'test_cache_collection'},
...
def mongo_cache_conn():
    mongo_config = KVDBConnectionConfig(
        host='localhost', port=27018, db_info={'db': 'test_cache_db', 'table': 'test_cache_collection'},
...
def mixed_cache_conn():
    redis_config = KVDBConnectionConfig(
        host='localhost', port=6381, need_to_clear=True, db_info={'db': 0, 'table': 'test_cache_collection'},
        params={'ss_name': 'sorted_node_pairs', 'hs_name': 'node_pairs', 'max_storage': 5e+8})
