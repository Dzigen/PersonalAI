...
def test_create_triplets(init_triplets: List[Triplet], add_nodes_flag: bool, expected_init_count: Dict[str, int],
    ...
    real_creation_info = embeddings_model.create_triplets(
        init_triplets, create_nodes=add_nodes_flag)
    ...
    if add_nodes_flag:
        ...
        for triplet in init_triplets:
            assert embeddings_model.vectordbs['nodes'].item_exist(
    ...
    for triplet in init_triplets:
        assert embeddings_model.vectordbs['triplets'].item_exist(
            triplet.relation.id)
    embeddings_model.clear()
...
def test_delete_triplets(init_triplets: List[Triplet], expected_creation_info: Dict[str, Set[str]], expected_init_count: Dict[str, int],
    ...
    real_creation_info = embeddings_model.create_triplets(
        init_triplets, create_nodes=True)
    assert real_creation_info == expected_creation_info
    ...
    flag = False
    try:
        embeddings_model.delete_triplets(
            triplets_to_delete, delete_info=delete_info)
    ...
    if not exception:
        ...
        for i, triplet in enumerate(triplets_to_delete):
            if delete_info[i]['triplet']:
                assert not embeddings_model.vectordbs['triplets'].item_exist(
                    triplet.relation.id)
            if delete_info[i]['s_node']:
                assert not embeddings_model.vectordbs['nodes'].item_exist(
                    triplet.start_node.id)
