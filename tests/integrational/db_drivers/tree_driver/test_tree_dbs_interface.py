...
def test_update(
    ...
    try:
        ...
        for expected_node in update_nodes:
            print("HERE 2")
            real_node = treedb_conn.read(
                [expected_node.id], ids_type=TreeIdType.external)[0]
...
def test_getleafdescendants(create_pairs: List[Tuple[str, TreeNode]], id: str, expected_descendants: Dict[str, TreeNode], exception: bool, treedb_conn: AbstractTreeDatabaseConnection):
    ...
    try:
        real_descendants = treedb_conn.get_leaf_descendants(
            id, id_type=TreeIdType.external)
    except ValueError:
        assert exception
    else:
        assert not exception
