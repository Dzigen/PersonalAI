...
for db_vendor in AVAILABLE_KV_DBS:
    for i in range(len(KVDB_CREATE_TEST_CASES)):
        KVDB_POPULATED_CREATE_TEST_CASES.append(
            KVDB_CREATE_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_KV_DBS:
    for i in range(len(KVDB_DELETE_TEST_CASES)):
        KVDB_POPULATED_DELETE_TEST_CASES.append(
            KVDB_DELETE_TEST_CASES[i] + [db_vendor])
...
KVDB_READ_TEST_CASES = [
    ...
    [[FULL_INSTANCE1, FULL_INSTANCE2], ['123', '789', '456'], {
        'exception': False, 'output_ids': ['123', None, '456']}],
...
for db_vendor in AVAILABLE_KV_DBS:
    for i in range(len(KVDB_READ_TEST_CASES)):
        KVDB_POPULATED_READ_TEST_CASES.append(
            KVDB_READ_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_KV_DBS:
    for i in range(len(KVDB_COUNT_TEST_CASES)):
        KVDB_POPULATED_COUNT_TEST_CASES.append(
            KVDB_COUNT_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_KV_DBS:
    for i in range(len(KVDB_EXIST_TEST_CASES)):
        KVDB_POPULATED_EXIST_TEST_CASES.append(
            KVDB_EXIST_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_KV_DBS:
    for i in range(len(KVDB_CLEAR_TEST_CASES)):
        KVDB_POPULATED_CLEAR_TEST_CASES.append(
            KVDB_CLEAR_TEST_CASES[i] + [db_vendor])
