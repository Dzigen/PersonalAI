...
from src.db_drivers.kv_driver import KeyValueDriver, KeyValueDriverConfig, KVDBConnectionConfig, MixedKVConnector
...
@pytest.fixture(scope='package')
def mixed_conn(mongo_conn, redis_conn):
    mixed_conn = MixedKVConnector()

    mixed_conn.mongo_conn = mongo_conn
    mixed_conn.config.params['mongo_config'] = mongo_conn.config

    mixed_conn.redis_conn = redis_conn
    mixed_conn.config.params['redis_config'] = redis_conn.config
