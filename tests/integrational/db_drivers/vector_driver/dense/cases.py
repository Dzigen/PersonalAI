...
# TO CHANGE
AVAILABLE_VECTOR_DBS = ['chroma', 'inmemory', 'opensearch', 'elasticsearch', 'weaviate', 'qdrant']  # 'chroma', 'inmemory', 'opensearch', 'elasticsearch', 'weaviate', 'qdrant' | 'milvus'
...
for db_vendor in AVAILABLE_VECTOR_DBS:
    for i in range(len(VECTORDB_CREATE_TEST_CASES)):
        VECTORDB_POPULATED_CREATE_TEST_CASES.append(
            VECTORDB_CREATE_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_VECTOR_DBS:
    for i in range(len(VECTORDB_DELETE_TEST_CASES)):
        VECTORDB_POPULATED_DELETE_TEST_CASES.append(
            VECTORDB_DELETE_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_VECTOR_DBS:
    for i in range(len(VECTORDB_READ_TEST_CASES)):
        VECTORDB_POPULATED_READ_TEST_CASES.append(
            VECTORDB_READ_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_VECTOR_DBS:
    for i in range(len(VECTORDB_UPSERT_TEST_CASES)):
        VECTORDB_POPULATED_UPSERT_TEST_CASES.append(
            VECTORDB_UPSERT_TEST_CASES[i] + [db_vendor])

###############################################################################################

VECTORDB_RETRIEVE_TEST_CASES = [
    # 1.1 ретрив по одному квери
    [[FULL_INSTANCE1, FULL_INSTANCE2], [FULL_INSTANCE1],
        1, None, {'exception': False, 'output_size': 1}],
    # 1.2 ретрив по одному квери (из подмножества)
    [[FULL_INSTANCE1, FULL_INSTANCE2], [FULL_INSTANCE1], 2, [
        FULL_INSTANCE1.id], {'exception': False, 'output_size': 1}],
    # 2. ретрив по нескольким квери
    [[FULL_INSTANCE1, FULL_INSTANCE2], [FULL_INSTANCE1, FULL_INSTANCE1],
        1, None, {'exception': False, 'output_size': 1}],
    # 3. в бд меньше элементов, чем заданное количество
    [[FULL_INSTANCE1], [FULL_INSTANCE1], 2, None,
        {'exception': False, 'output_size': 1}],
    # 4. torch-тип данных эмбеддинга
    [[FULL_INSTANCE1, FULL_INSTANCE2], [INSTANCE_WITH_TORCH_EMB],
        2, None, {'exception': True, 'output_size': -1}],
    # 5. numpy-тип данных эмбеддинга
    [[FULL_INSTANCE1, FULL_INSTANCE2], [INSTANCE_WITH_NUMPY_EMB],
        2, None, {'exception': True, 'output_size': -1}],
    # 6. неверный формат ембеддинга квери # 1
    [[FULL_INSTANCE1, FULL_INSTANCE2], [INSTANCE_WITH_BAD_EMB1],
        2, None, {'exception': True, 'output_size': -1}],
...
for db_vendor in AVAILABLE_VECTOR_DBS:
    for i in range(len(VECTORDB_RETRIEVE_TEST_CASES)):
        VECTORDB_POPULATED_RETRIEVE_TEST_CASES.append(
            VECTORDB_RETRIEVE_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_VECTOR_DBS:
    for i in range(len(VECTORDB_COUNT_TEST_CASES)):
        VECTORDB_POPULATED_COUNT_TEST_CASES.append(
            VECTORDB_COUNT_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_VECTOR_DBS:
    for i in range(len(VECTORDB_EXIST_TEST_CASES)):
        VECTORDB_POPULATED_EXIST_TEST_CASES.append(
            VECTORDB_EXIST_TEST_CASES[i] + [db_vendor])
...
for db_vendor in AVAILABLE_VECTOR_DBS:
    for i in range(len(VECTORDB_CLEAR_TEST_CASES)):
        VECTORDB_POPULATED_CLEAR_TEST_CASES.append(
            VECTORDB_CLEAR_TEST_CASES[i] + [db_vendor])
