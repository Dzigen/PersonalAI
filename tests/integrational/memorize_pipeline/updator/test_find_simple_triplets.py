...
@pytest.mark.parametrize("kg_triplets, triplets, agent_stub_answers, expected_obsolete_ids", [
    ...
    (INIT_KNOWLEDGE_GRAPH, [TEST_SIMPLE_TRIPLET4],
     [[TRIPLET_ANSWER1]], [[SIMPLE_TRIPLET1.id]]),
    # 3. найдено несколько устаревших триплетов (разные замены)
    (INIT_KNOWLEDGE_GRAPH, [TEST_SIMPLE_TRIPLET4, TEST_SIMPLE_TRIPLET5], [
     [TRIPLET_ANSWER1], [TRIPLET_ANSWER2]], [[SIMPLE_TRIPLET1.id], [SIMPLE_TRIPLET3.id]]),
    # 4. найдено несколько устаревших триплетов (итеративная замена того же ребра)
    (INIT_KNOWLEDGE_GRAPH, [TEST_SIMPLE_TRIPLET4, TEST_SIMPLE_TRIPLET6], [
     [TRIPLET_ANSWER1], [TRIPLET_ANSWER3]], [[SIMPLE_TRIPLET1.id], [TEST_SIMPLE_TRIPLET4.id]]),
