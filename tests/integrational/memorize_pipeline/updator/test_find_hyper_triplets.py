...
@pytest.mark.parametrize("kg_triplets, triplets, agent_stub_answers, expected_obsolete_ids", [
    ...
    (INIT_KNOWLEDGE_GRAPH, [TEST_HYPER_TRIPLET4],
     [[HYPER_ANSWER1]], [[HYPER_TRIPLET2.id]]),
    # 3. найдено несколько устаревших триплетов (разные замены)
    (INIT_KNOWLEDGE_GRAPH, [TEST_HYPER_TRIPLET4, TEST_HYPER_TRIPLET5], [
     [HYPER_ANSWER1], [HYPER_ANSWER2]], [[HYPER_TRIPLET2.id], [HYPER_TRIPLET3.id]]),
    # 4. найдено несколько устаревших триплетов (итеративная замена того же ребра)
    (INIT_KNOWLEDGE_GRAPH, [TEST_HYPER_TRIPLET4, TEST_HYPER_TRIPLET6], [
     [HYPER_ANSWER1], [HYPER_ANSWER3]], [[HYPER_TRIPLET2.id], [TEST_HYPER_TRIPLET4.id]]),
...
def test_find_hyper(llm_updator: LLMUpdator, kg_triplets: List[Triplet], triplets: List[Triplet],
    ...
    for triplet, stub_answer, expected_output in zip(triplets, agent_stub_answers, expected_obsolete_ids):
        ...
        real_obsolete_ids, _, _ = llm_updator.find_hyper_obsolete_triplet_ids(
            triplet)
        ...
        if len(real_obsolete_ids) > 0:
            obsolete_triplets = llm_updator.kg_model.graph_struct.db_conn.read(
                real_obsolete_ids)
