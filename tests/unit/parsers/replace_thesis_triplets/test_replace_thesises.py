...
AVAILABLE_RTHESIS_VERSIONS = {
    'v1': RTHESIS_PARSE_V1_TEST_CASES,
}
...
def test_custom_parse(raw_response: str, lang: str, expected_output: Dict[str, Set[str]],
                      exception: bool, rthesis_tconfig):
    try:
        real_output = rthesis_tconfig[lang].parse_answer_func(raw_response)
    except Exception:
        assert exception
    else:
...
@pytest.mark.parametrize("base_triplet, incident_triplets, expected_output, exception", REPLACETHESIS_FORMATE_TEST_CASE)
...
@pytest.mark.parametrize("parsed_response, base_triplet, incident_triplets, expected_output, exception", REPLACETHESIS_POSTPROCESS_TEST_CASE)
