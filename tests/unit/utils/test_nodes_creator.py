...
@pytest.mark.parametrize("params, expected", [
    ...
    ({'add_stringified_node': True, 'name': TEST_NODE_NAME,
     'prop': {}}, {'str': TEST_NODE_NAME}),
    # не добавлять строковое представление в вершину
    ({'add_stringified_node': False, 'name': TEST_NODE_NAME,
     'prop': {}}, {'str': TEST_NODE_NAME}),
    # строковое представление с отметкой времени
    ({'add_stringified_node': True, 'name': TEST_NODE_NAME, 'prop': {
     'time': TEST_TIME}}, {'str': f"{TEST_TIME}: {TEST_NODE_NAME}"}),
    # строковое представление без отметки времени
    ({'add_stringified_node': True, 'name': TEST_NODE_NAME,
     'prop': {}}, {'str': TEST_NODE_NAME}),
    # строкое представление со свойствами
    ({'add_stringified_node': True, 'name': TEST_NODE_NAME, 'prop': {'p1': STRING_PROP_VALUE,
     'p2': INT_PROP_VALUE}}, {'str': f"{TEST_NODE_NAME} (p1: {STRING_PROP_VALUE}; p2: {INT_PROP_VALUE})"}),
    ...
    ({'add_stringified_node': True, 'name': TEST_NODE_NAME, 'prop': {'name': STRING_PROP_VALUE, 'type': STRING_PROP_VALUE,
     'raw_time': STRING_PROP_VALUE, 'time': TEST_TIME, 'str_id': STRING_PROP_VALUE}}, {'str': f"{TEST_TIME}: {TEST_NODE_NAME}"}),
])
